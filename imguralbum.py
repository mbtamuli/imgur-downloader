...
class ImgurAlbumDownloader:
    def __init__(self, album_url):
        ...
        self.pya2c = None
        if aria2c.isAria2Installed():
            self.pya2c = aria2c.PyAria2()
    ...
    def save_images(self, foldername=False):
        ...
        for (counter, image) in enumerate(self.imageIDs, start=1):
            ...
            if os.path.isfile(path):
                ...
                try:
                    if self.pya2c:
                        self.pya2c.addUri([image_url], options={'dir': albumFolder, 'out': path})
                    else:
